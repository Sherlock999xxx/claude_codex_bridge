...
def test_load_resume_session_id_rejects_missing_bound_session_path(tmp_path: Path) -> None:
    ccb_dir = tmp_path / ".ccb"
    agent_dir = ccb_dir / "agents" / "agent1" / "runtime"
    agent_dir.mkdir(parents=True, exist_ok=True)
    session_file = ccb_dir / ".codex-agent1-session"
    session_file.write_text(
        json.dumps(
            {
                "codex_session_id": "sid-missing",
                "codex_session_root": str(ccb_dir / "agents" / "agent1" / "provider-state" / "codex" / "home" / "sessions"),
                "codex_session_path": str(ccb_dir / "agents" / "agent1" / "provider-state" / "codex" / "home" / "sessions" / "missing.jsonl"),
            }
        ),
        encoding="utf-8",
    )
